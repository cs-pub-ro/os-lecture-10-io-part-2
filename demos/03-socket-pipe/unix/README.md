# Bidirection Transfer with Unix Sockets

This example demonstrates a bidirectional transfer using a pair of network sockets part of the same connection.
(Unix) Sockets are bidirectional, so only a connection with two sockets is required: one for each side - client and server.
...
A message is being sent from the client to server using the socket connection available as a filesystem entry: `socket_channel`.
